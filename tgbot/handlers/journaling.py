...
from tgbot.keyboards.reply import choose_jour, self_hi, work_ans_kb, sub_hi, mkb
...
async def year_hi(message: types.Message, state: FSMContext, session: AsyncSession):
  await message.answer(data.jour.year.hi.text.format(await count_questions_in_category(session, category='year')), reply_markup=mkb(data.jour.year.hi.kb))
  await Jour.year_hi.set()
